...
const RecruiterJDDetails = () => {
  ...
  const filteredSwitcherJDs = useMemo(() => {
    ...
    return extendedJDs.filter(item => 
  ...
  const handleJdChange = (newJdId) => {
    navigate(`/recruiter/jd/${newJdId}?filter=shortlisted`);
  ...
  const [candidates, setCandidates] = useState(() => {
    const filtered = mockCandidates.filter(c => c.jdId === (id || "JD-UX2026-001"));
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      default: return 'bg-brand-purple/10 text-brand-purple border-brand-purple/20';
    }
  };
      ...
      const newIngestions = [
        {
          ...
          email: "amit.wadhwa.live@scanjd-ingest.com",
        ...
        },
        {
          ...
          status: "Newly Screened",
          email: "sonal.mehta.live@scanjd-ingest.com",
          ...
          {filterType === 'shortlisted' ? (
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs font-bold text-brand-purple uppercase tracking-wider">Shortlisted Candidates</span>
              </div>
              <h1 className="text-2xl font-bold text-gray-900">{jd.title}</h1>
              <p className="text-xs text-gray-500 mt-0.5">Shortlisted candidate pool for this Job Description</p>
            </div>
          ) : (
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-brand-purple uppercase tracking-wider">{jd.id}</span>
          ...
          {filterType === 'shortlisted' && (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-3 px-4 py-2 bg-white border border-gray-200 rounded-xl shadow-sm hover:border-brand-purple/40 hover:bg-brand-purple/5 transition-all text-left min-w-[240px] cursor-pointer"
              >
                <div className="flex-1 min-w-0">
                  <div className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider leading-none">Switch JD</div>
                  <div className="text-sm font-bold text-gray-800 truncate mt-0.5">{jd.title}</div>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${getStatusColor(jd.status)}`}>{jd.status}</span>
                  ...
                  <div className="absolute top-full right-0 mt-2 z-20 w-80 bg-white border border-gray-200 rounded-2xl shadow-xl py-2 max-h-80 overflow-y-auto flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto max-h-56">
                      {filteredSwitcherJDs.length === 0 ? (
                        ...
                        filteredSwitcherJDs.map(item => (
                          <button
                            key={item.id}
                            type="button"
                            onClick={() => handleJdChange(item.id)}
                            className={`w-full flex items-start gap-3 px-3 py-2.5 text-left transition-colors hover:bg-gray-50 cursor-pointer ${jd.id === item.id ? 'bg-brand-purple/5' : ''}`}
                          >
                            <div className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${
                              item.status === 'Active' ? 'bg-emerald-500' :
                              item.status === 'Reviewing' ? 'bg-amber-500' :
                              item.status === 'Interviewing' ? 'bg-brand-purple' :
                              'bg-gray-300'
                            }`} />
                            <div className="min-w-0 flex-1">
                              <div className={`text-xs font-bold truncate ${jd.id === item.id ? 'text-brand-purple' : 'text-gray-800'}`}>{item.title}</div>
                              <div className="flex items-center gap-2 mt-0.5">
                                <span className="text-[10px] text-gray-400 font-mono">{item.id}</span>
                                <span className="text-[10px] text-gray-400">·</span>
                                <span className="text-[10px] text-gray-400 flex items-center gap-1">
                                  <Building2 className="w-2.5 h-2.5" />{item.company}
                                </span>
                              </div>
                            </div>
                            <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded border flex-shrink-0 ${getStatusColor(item.status)}`}>{item.status}</span>
                          </button>
                        ))
                      )}
                    </div>
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
            <div className="space-y-4 flex-1">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="px-2.5 py-0.5 bg-brand-purple/10 text-brand-purple rounded-lg text-[11px] font-bold flex items-center gap-1 border border-brand-purple/20">
              ...
              <div className="space-y-2 pt-1">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {jd.skillsRequired && jd.skillsRequired.map(skill => (
                    <span key={skill} className="px-2.5 py-1 bg-brand-purple/5 border border-brand-purple/10 text-brand-purple rounded-xl text-xs font-bold shadow-sm flex items-center gap-1.5">
            ...
            <div className="w-full md:w-80 bg-gray-50/50 rounded-2xl border border-gray-200/80 overflow-hidden flex flex-col justify-between">
              {/* Header */}
